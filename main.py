...
def generate_path(num_cities, start, pheromones, distances):
    ...
    for _ in range(1, num_cities):
        ...
        next_edges = np.delete(distances[path[-1]], path) # Get distances to all potential next nodes
        heuristic = np.divide(np.divide(next_pheromones, next_edges), np.divide(np.sum(next_pheromones), np.sum(next_edges))) # Calculate heuristic value
       
        path = np.append(path, random.choices(unvisited, weights=heuristic, cum_weights=None,k=1))  # Picks a next city at random, but weighted
    return path
...
def update_pheromones(reward, path, n):
    ...
    """
    pheromones_update = np.zeros((len(path), len(path)), dtype='longdouble')
    x_indices = path[:-1]
    y_indices = path[1:]
    pheromones_update[x_indices, y_indices] += np.longdouble(reward)
...
def tests():
    ...
    test_dic = {'1': [10, 0.25, 0.25], '2': [10, 0.5, 0.5], '3': [10, 1.5, 1.5],
                '4': [50, 0.25, 0.25], '5': [50, 0.5, 0.5], '6': [50, 1.5, 1.5],
                '7': [100, 0.25, 0.25], '8': [100, 0.5, 0.5], '9': [100, 1.5, 1.5]}      
    ...
    for i, ax in enumerate(fig.axes):
        ...
        for j in tqdm(range(10)):
            fitness_list.append(main(distances, num_cities, row[0], row[1], row[2]))
        averages = [sum(x) / len(x) for x in zip(*fitness_list)]
        ax.set_title(label="M=%s, E=%s, Q=%s | Best=%s" % (row[0], row[1], row[2], int(min(averages))))
        ax.plot(np.arange(len(averages)), averages)

    plt.show()

def main(distances, num_cities, M, E, Q):
    ...
    for _ in (range(iterations)):
        ...
        for _ in range(M):
            path = generate_path(num_cities, start_node, pheromones, distances)
        ...
        pheromones = np.add(pheromones, pheromone_update) * E
